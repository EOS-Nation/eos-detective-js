...
export async function transfers(
  ...
  queryParams.accumulated =
    options.accumulated === undefined ? true : options.accumulated;
  queryParams.excludes = options.excludes || [];
  queryParams.quantity_min =
    options.quantity_min === undefined
      ? 1000
      : convertQuantity(options.quantity_min, options.chain, true);
  queryParams.time_min =
    options.time_min === undefined
      ? yesterday.toISOString()
      : options.time_min.toISOString();
  queryParams.offset = options.offset || 0;
  queryParams.limit = options.limit || 1000;

  // optional params
  if (options.quantity_max) {
    queryParams.quantity_max = convertQuantity(
  ...
  if (options.accumulated_min) {
    queryParams.accumulated_min = convertQuantity(
      options.accumulated_min,
      options.chain,
      true
    );
  }
  if (options.accumulated_max) {
    queryParams.accumulated_max = convertQuantity(
      options.accumulated_max,
      options.chain,
      true
    );
  }
  ...
  try {
    const res = await Axios.request<ApiResponse<TransfersData>>({
      ...
      transformResponse: (data: any) => {
        ...
        if (json.error) {
          return new ApiResponse<TransfersData>(undefined, {
            code: json.code,
        ...
        transfersData.forEach((transfer) => {
          if (
            !accountsData.some((account) => account.name === transfer.to.name)
          ) {
            accountsData.push(transfer.to);
          }
          if (
            !accountsData.some((account) => account.name === transfer.from.name)
          ) {
